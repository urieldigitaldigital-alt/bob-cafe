...
export interface CoverflowCarouselProps {
  ...
  /** Called whenever the centred slide changes (drag settle or navigation). */
  onSelectedChange?: (index: number) => void;
  /** Called when the already-centred card is clicked — e.g. to open detail. */
    ...
    >
      <div className="relative">
        <div
          ...
          className="cursor-grab overflow-hidden py-10 outline-none ring-primary focus-visible:ring-2 active:cursor-grabbing"
          style={{
          ...
          >
            {slides.map((slide, index) => (
              <div
                ...
                className={cn(
                  "absolute top-0 left-1/2 aspect-square cursor-pointer overflow-hidden rounded-2xl bg-secondary shadow-xl will-change-transform",
                  cardClassName,
                )}
      ...
      {showCaption && active?.title && (
        ...
        >
          ...
          {active.subtitle && (
            <p className="mt-1 text-[13px] text-foreground/60">{active.subtitle}</p>
          )}
